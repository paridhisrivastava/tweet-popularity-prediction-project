...
def main():
    ...
    #target class is the retweet based popularity labelled column
    train_target = numpy.ravel(traindata[1:, [18]])
    ...
    plt.suptitle('Significance of Features in Prediction of Popularity by Retweet Count', fontsize=14)
    plt.xlabel('Features', fontsize=12)
    plt.ylabel('Feature values', fontsize=12)
    plt.savefig('retweets_feature_importance.png')

    #apply the RandomForest classification model on the test data to predict the popularity class label
    test_target = numpy.ravel(testdata[1:, [18]])
